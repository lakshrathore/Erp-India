...
export function EmployeeFormPage() {
  ...
  const { data: depts = [] } = useQuery({ queryKey: ['departments'], queryFn: async () => { const { data } = await api.get('/payroll/departments'); return data.data } })
  const { data: desigs = [] } = useQuery({ queryKey: ['designations'], queryFn: async () => { const { data } = await api.get('/payroll/designations'); return data.data } })
  const { data: structures = [] } = useQuery({ queryKey: ['salary-structures'], queryFn: async () => { const { data } = await api.get('/payroll/salary-structures'); return data.data } })
